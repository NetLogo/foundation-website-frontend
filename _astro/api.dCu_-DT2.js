...
`)))!==null&&t!==void 0?t:!1}const de="Accept",X="Content-Type",W="application/json",be="application/graphql-response+json",pe=e=>e.replace(/([\s,]|#[^\n\r]+)+/g," ").trim(),Dt=e=>{const t=e.toLowerCase();return t.includes(be)||t.includes(W)},fe=e=>{try{if(Array.isArray(e))return{_tag:"Batch",executionResults:e.map(me)};if(P(e))return{_tag:"Single",executionResult:me(e)};throw new Error(`Invalid execution result: result is not object or array. 
Got:
${String(e)}`)}catch(t){return t}},me=e=>{if(typeof e!="object"||e===null)throw new Error("Invalid execution result: result is not object");let t,n,s;if("errors"in e){if(!P(e.errors)&&!Array.isArray(e.errors))throw new Error("Invalid execution result: errors is not plain object OR array");t=e.errors}if("data"in e){if(!P(e.data)&&e.data!==null)throw new Error("Invalid execution result: data is not plain object");n=e.data}if("extensions"in e){if(!P(e.extensions))throw new Error("Invalid execution result: extensions is not plain object");s=e.extensions}return{data:n,errors:t,extensions:s}},vt=e=>e._tag==="Batch"?e.executionResults.some(Ee):Ee(e.executionResult),Ee=e=>Array.isArray(e.errors)?e.errors.length>0:!!e.errors,Ce=e=>typeof e=="object"&&e!==null&&"kind"in e&&e.kind===l.OPERATION_DEFINITION,At=e=>{let t;const n=e.definitions.filter(Ce);return n.length===1&&(t=n[0].name?.value),t},St=e=>{let t=!1;const n=e.definitions.filter(Ce);return n.length===1&&(t=n[0].operation==="mutation"),t},Y=(e,t)=>{const n=typeof e=="string"?e:It(e);let s=!1,i;if(t)return{expression:n,isMutation:s,operationName:i};const r=we(()=>typeof e=="string"?pt(e):e);return r instanceof Error?{expression:n,isMutation:s,operationName:i}:(i=At(r),s=St(r),{expression:n,operationName:i,isMutation:s})},te=JSON,H=async e=>{const t={...e,method:e.request._tag==="Single"?e.request.document.isMutation?"POST":re(e.method??"post"):e.request.hasMutations?"POST":re(e.method??"post"),fetchOptions:{...e.fetchOptions,errorPolicy:e.fetchOptions.errorPolicy??"none"}},s=await Ct(t.method)(t);if(!s.ok)return new k({status:s.status,headers:s.headers},{query:e.request._tag==="Single"?e.request.document.expression:e.request.query,variables:e.request.variables});const i=await bt(s,e.fetchOptions.jsonSerializer??te);if(i instanceof Error)throw i;const r={status:s.status,headers:s.headers};if(vt(i)&&t.fetchOptions.errorPolicy==="none"){const a=i._tag==="Batch"?{...i.executionResults,...r}:{...i.executionResult,...r};return new k(a,{query:e.request._tag==="Single"?e.request.document.expression:e.request.query,variables:e.request.variables})}switch(i._tag){case"Single":return{...r,...Ne(t)(i.executionResult)};case"Batch":return{...r,data:i.executionResults.map(Ne(t))};default:Z(i)}},Ne=e=>t=>({extensions:t.extensions,data:t.data,errors:e.fetchOptions.errorPolicy==="all"?t.errors:void 0}),bt=async(e,t)=>{const n=e.headers.get(X),s=await e.text();return n&&Dt(n)?fe(t.parse(s)):fe(s)},Ct=e=>async t=>{const n=new Headers(t.headers);let s=null,i;n.has(de)||n.set(de,[be,W].join(", ")),e==="POST"?(i=(t.fetchOptions.jsonSerializer??te).stringify(kt(t)),typeof i=="string"&&!n.has(X)&&n.set(X,W)):s=wt(t);const r={method:e,headers:n,body:i,...t.fetchOptions};let a=new URL(t.url),h=r;if(t.middleware){const u=await Promise.resolve(t.middleware({...r,url:t.url,operationName:t.request._tag==="Single"?t.request.document.operationName:void 0,variables:t.request.variables})),{url:p,...E}=u;a=new URL(p),h=E}return s&&s.forEach((u,p)=>{a.searchParams.append(p,u)}),await(t.fetch??fetch)(a,h)},kt=e=>{switch(e.request._tag){case"Single":return{query:e.request.document.expression,variables:e.request.variables,operationName:e.request.document.operationName};case"Batch":return ye(e.request.query,e.request.variables??[]).map(([t,n])=>({query:t,variables:n}));default:throw Z(e.request)}},wt=e=>{const t=e.fetchOptions.jsonSerializer??te,n=new URLSearchParams;switch(e.request._tag){case"Single":return n.append("query",pe(e.request.document.expression)),e.request.variables&&n.append("variables",t.stringify(e.request.variables)),e.request.document.operationName&&n.append("operationName",e.request.document.operationName),n;case"Batch":{const s=e.request.variables?.map(a=>t.stringify(a))??[],i=e.request.query.map(pe),r=ye(i,s).map(([a,h])=>({query:a,variables:h}));return n.append("query",t.stringify(r)),n}default:throw Z(e.request)}};class Rt{url;requestConfig;constructor(t,n={}){this.url=t,this.requestConfig=n}rawRequest=async(...t)=>{const[n,s,i]=t,r=Fe(n,s,i),{headers:a,fetch:h=globalThis.fetch,method:d="POST",requestMiddleware:u,responseMiddleware:p,excludeOperationName:E,...f}=this.requestConfig,{url:I}=this;r.signal!==void 0&&(f.signal=r.signal);const N=Y(r.query,E),y=await H({url:I,request:{_tag:"Single",document:N,variables:r.variables},headers:{...b($(a)),...b(r.requestHeaders)},fetch:h,method:d,fetchOptions:f,middleware:u});if(p&&await p(y,{operationName:N.operationName,variables:s,url:this.url}),y instanceof Error)throw y;return y};async request(t,...n){const[s,i]=n,r=Lt(t,s,i),{headers:a,fetch:h=globalThis.fetch,method:d="POST",requestMiddleware:u,responseMiddleware:p,excludeOperationName:E,...f}=this.requestConfig,{url:I}=this;r.signal!==void 0&&(f.signal=r.signal);const N=Y(r.document,E),y=await H({url:I,request:{_tag:"Single",document:N,variables:r.variables},headers:{...b($(a)),...b(r.requestHeaders)},fetch:h,method:d,fetchOptions:f,middleware:u});if(p&&await p(y,{operationName:N.operationName,variables:r.variables,url:this.url}),y instanceof Error)throw y;return y.data}async batchRequests(t,n){const s=Le(t,n),{headers:i,excludeOperationName:r,...a}=this.requestConfig;s.signal!==void 0&&(a.signal=s.signal);const h=s.documents.map(({document:f})=>Y(f,r)),d=h.map(({expression:f})=>f),u=h.some(({isMutation:f})=>f),p=s.documents.map(({variables:f})=>f),E=await H({url:this.url,request:{_tag:"Batch",operationName:void 0,query:d,hasMutations:u,variables:p},headers:{...b($(i)),...b(s.requestHeaders)},fetch:this.requestConfig.fetch??globalThis.fetch,method:this.requestConfig.method||"POST",fetchOptions:a,middleware:this.requestConfig.requestMiddleware});if(this.requestConfig.responseMiddleware&&await this.requestConfig.responseMiddleware(E,{operationName:void 0,variables:p,url:this.url}),E instanceof Error)throw E;return E.data}setHeaders(t){return this.requestConfig.headers=t,this}setHeader(t,n){const{headers:s}=this.requestConfig;return s?s[t]=n:this.requestConfig.headers={[t]:n},this}setEndpoint(t){return this.url=t,this}}const Lt=(e,t,n)=>e.document?e:{document:e,variables:t,requestHeaders:n,signal:void 0},x=(e,...t)=>e.reduce((n,s,i)=>`${n}${s}${i in t?String(t[i]):""}`,""),O={introduction:x`
    query {
      introduction {
        title
        description
